  ...
  return (
    <nav className="z-10 md:fixed inset-0 flex flex-col justify-between items-start gap-14 w-full md:h-full p-4 max-md:pt-12 pointer-events-none">
      ...
      <div className="pointer-events-auto">
        <AnimText as={'h1'} className="text-4xl">
          {BIO.name}
        </AnimText>
        <AnimText as={'p'} className="text-2xl">
          {BIO.title}
        </AnimText>
      </div>
      ...
      <div className="flex items-end gap-12 w-1/2 max-md:w-full pointer-events-auto">
        {/* Contact Section */}
        <AnimIn
          onMouseEnter={() => setShowContact(true)}
          onMouseLeave={() => setShowContact(false)}
          className="relative flex flex-col font-sec uppercase tracking-wider"
        >
          <MoveDown
            size={20}
            strokeWidth={1}
        ...
        <AnimIn
          ...
          className="group z-60 relative flex flex-col cursor-pointer"
        >
          ...
          <AnimatePresence>
            {showSkills && (
              <motion.div
                ...
                exit={{ height: 0, opacity: 0 }}
                transition={{ duration: 0.5, ease: easings.cubiz }}
                className="max-md:hidden overflow-hidden mt-4"
              >
                <div className="gap-8 grid xl:grid-cols-4 mb-2">
                  {SKILLS.map((set, i) => (
                    <div key={i}>
        ...
        {/* clock Section */}
        <div className="flex justify-end w-full duration-300 me-4">
          <DigitalClock />
