...
const BgDitherVeilFragmentShader = `
precision highp float;
...
vec3 BgDitherVeil(vec2 uv, vec3 color) {
...
void mainImage(in vec4 inputColor, in vec2 uv, out vec4 outputColor) {
  ...
  color.rgb = BgDitherVeil(uv, color.rgb);
  outputColor = color;
}
`
...
class RetroEffectImpl extends Effect {
  ...
  constructor() {
    ...
    super('RetroEffect', BgDitherVeilFragmentShader, { uniforms })
...
function BgDitherVeiledWaves({
  ...
  const uniforms = useState<WaveUniforms>(initialUniforms)[0]
  const waveUniformsRef = useRef<WaveUniforms>(initialUniforms)
  ...
  }, [size, gl, uniforms.resolution.value])
...
export default function BgDitherVeil({
  waveSpeed = 0.05,
  waveFrequency = 5,
  waveAmplitude = 0.1,
  // waveColor = [0.5, 0.5, 0.5],
  // waveColor = [0.9803921568627451, 0.10980392156862746, 0.1470588235294118],
  waveColor = [0.55, 0.55, 0.67],
  colorNum = 33,
  pixelSize = 1,
  ...
  return (
    <Canvas
      ...
      className="inset-0 max-w-dvw max-h-dvh opacity-50 mix-blend-difference fixed!"
    >
      <BgDitherVeiledWaves
        waveSpeed={waveSpeed}
