...
const TextScramble: React.FC<TextScramble> = ({
  text,
  scrambleSpeed = 50,
  maxIterations = 10,
  ...
  return (
    <motion.span
      ...
      className={`inline-block whitespace-pre-wrap ${className} ${isScrambling ? 'cursor-default' : 'cursor-pointer'}`}
      {...props}
    >
      <span className="sr-only">{displayText}</span>
      <span aria-hidden="true">
        {displayText.split('').map((char, index) => (
          <span key={index} className={`${revealedIndices.has(index) || !isScrambling || !isHovering ? className : scrambledClassName}`}>
            {char}
          </span>
        ))}
      </span>
